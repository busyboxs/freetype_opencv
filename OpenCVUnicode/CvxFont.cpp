...
void cvx::CvxFont::putWChar(cv::Mat& img, uint32_t wc, cv::Point& pos, const cv::Scalar& color)
{
    ...
    if (vertical)
    {
        ...
        m_maxDiffHeight = std::max(m_maxDiffHeight, static_cast<long>(rows - (slot->metrics.vertBearingY >> 6)));
    }
    else
    {
        gPos.x += (slot->metrics.horiBearingX >> 6);
        gPos.y -= (slot->metrics.horiBearingY >> 6);
        m_maxDiffHeight = std::max(m_maxDiffHeight, static_cast<long>(rows - (slot->metrics.horiBearingY >> 6)));
    }
    ...
    for (unsigned i = 0; i < rows; ++i)
    {
        ...
        {
            auto off = i * slot->bitmap.pitch + j / 8;
